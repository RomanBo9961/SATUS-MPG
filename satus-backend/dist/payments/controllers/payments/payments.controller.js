...
let PaymentsController = class PaymentsController {
    ...
    async createCheckoutSession(body) {
        ...
        const priceAmountCentavos = targetLicense === 'AVANZADO' ? 320 : 500;
        try {
            ...
            throw new common_1.UnauthorizedException('La aduana bancaria rechazó la solicitud de pasarela.');
        }
    }
    async handleStripeWebhook(sig, req) {
        ...
        if (event.type === 'checkout.session.completed') {
            ...
            const finalResult = await this.authService.processLicenseUpgrade(userId, guestId, targetLicense);
